...
def _get_subtitle_source():
    try:
        return load_key("subtitle_source")
    except Exception:
        return "whisper"


def _build_text_steps():
    # QUAN TRỌNG: rẽ nhánh theo subtitle_source (ocr/whisper), KHÔNG theo
    # sourceLang nữa. Trước đây chỉ check sourceLang=="zh" để quyết định gọi
    # zh_pipeline, khiến 2 việc không liên quan bị dính chùm vào nhau:
    #   - Whisper + tiếng Trung bị ép chạy VAD Silero + faster-whisper riêng
...
def main():
    ...
    parser.add_argument(
        ...
        help="Âm lượng nhạc nền gốc khi mix với giọng dub, 0.0-1.0 (VD: 0 = xoá "
             ...
             "dub (không áp dụng sub-only, vì không có giọng dub mới để mix). Rỗng "
...
def _finalize_result(output_path: str):
    ...
    # QUAN TRỌNG: từ khi mỗi job chạy với cwd = workDir riêng (thay vì cwd cố định
    # = videoLingoDir như trước), --output Java truyền vào (tuyệt đối, dạng
    # workDir/output/output_dub.mp4) và final_video (tương đối "output/output_dub.mp4")
    # CÓ THỂ trỏ CHÍNH XÁC cùng 1 file vật lý - shutil.copy2() sẽ ném SameFileError
    # nếu cố copy 1 file đè lên chính nó. Trước đây (cwd cố định = videoLingoDir) 2
    # đường dẫn này luôn là 2 file khác nhau nên chưa từng gặp lỗi này.
    final_video_abs = os.path.abspath(final_video)
    output_path_abs = os.path.abspath(output_path)
    if final_video_abs != output_path_abs:
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        shutil.copy2(final_video, output_path)
    print(f"RESULT_PATH={output_path}", flush=True)
