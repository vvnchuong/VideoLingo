...
def concat_or_convert(paths: list[str], final_output: str) -> None:
    if len(paths) == 1:
        try:
            subprocess.run(
                ["ffmpeg", "-y", "-i", paths[0], final_output],
                check=True, capture_output=True, text=True,
            )
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"ffmpeg convert lỗi: {e.stderr}") from e
        return
...
def main():
    ...
    parser.add_argument("--rate", default="1.0",
                         help="Tốc độ đọc 0.5-2.0 - xử lý bằng ffmpeg atempo sau khi có audio.")
    parser.add_argument("--list-voices", action="store_true")
    args = parser.parse_args()

    if args.list_voices:
        import json
        print(json.dumps(sorted(PRESET_VOICES)))
        return
    ...
    if args.voice not in PRESET_VOICES:
        print(f"LỖI: Không tìm thấy giọng '{args.voice}'. "
              f"Các giọng có sẵn: {', '.join(sorted(PRESET_VOICES))}", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        batch_dir = out_path.parent / f".vieneu_batch_{out_path.stem}"
        batch_dir.mkdir(parents=True, exist_ok=True)

        chunk_paths = []
        try:
            for i, chunk in enumerate(chunks):
                chunk_path = batch_dir / f"chunk_{i:03d}.wav"
                vieneu_tts(chunk, str(chunk_path), voice=args.voice)
                chunk_paths.append(str(chunk_path))

            concat_or_convert(chunk_paths, str(raw_output))
        finally:
