...
#         rprint(f"[green]📥 Using WHISPER model from HuggingFace:[/green] {model_name} ...")
#
#     # Hạ ngưỡng VAD (mặc định 0.500/0.363) vì giọng nói nhỏ bị nhạc nền che dễ
#     # không đạt onset mặc định -> cả đoạn bị coi là "im lặng" và bị loại khỏi
#     # list segment đưa vào Whisper, dẫn tới mất transcript hoàn toàn ở đoạn đó
#     # dù không có exception nào (VAD "không nghe thấy gì" là kết quả hợp lệ).
...
#         return full_audio[start_sample:end_sample]
#
#     # raw_audio_file được giữ trong signature (vẫn nhận từ _2_asr.py) nhưng
#     # không còn dùng để transcribe nữa — xem lý do ở bước 1 bên dưới.
#     vocal_audio_segment = load_audio_segment(vocal_audio_file, start, end)
#
#     # -------------------------
#     # 1. transcribe vocal audio (Demucs-separated when available)
#     # -------------------------
#     # Dùng vocal_audio_segment thay vì raw_audio_segment: nếu transcribe trên
#     # raw audio (còn nhạc nền), VAD nội bộ của WhisperX (vad_onset/vad_offset)
#     # dễ coi giọng nói nhỏ/bị nhạc che là "không có tiếng nói" và bỏ luôn cả
#     # đoạn trước khi kịp đưa vào model — nâng cấp model (vd large-v3) không
#     # sửa được vì đoạn audio đó chưa bao giờ tới được bước decode.
#     # Khi demucs=false, vocal_audio_file == raw_audio_file (xem _2_asr.py) nên
#     # hành vi không đổi cho video không có nhạc nền.
#     transcribe_start_time = time.time()
#     rprint("[bold green]Note: You will see Progress if working correctly ↓[/bold green]")
#     result = model.transcribe(vocal_audio_segment, batch_size=batch_size, print_progress=True)
#     transcribe_time = time.time() - transcribe_start_time
#     rprint(f"[cyan]⏱️ time transcribe:[/cyan] {transcribe_time:.2f}s")
#
#     # Free GPU resources
